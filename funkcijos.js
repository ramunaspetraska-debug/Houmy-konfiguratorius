...
function detectBackSides(groupModules) {
    ...
    groupModules.forEach(m => {
        let base = (typeof rawModels !== 'undefined' && rawModels[m.dataset.collection])
            ? rawModels[m.dataset.collection].find(x => x.id === m.dataset.id) : null;
...
function updateDimensions() {
    ...
    groups.forEach((g, index) => {
        ...
        if (dimState > 0) {
            ...
            let back = detectBackSides(g);
            let lineY = back.widthOnTop ? topY : bottomY;
            let textY = back.widthOnTop ? (lineY - 6) : (lineY + 16);
            let lineX = back.depthOnLeft ? leftX : rightX;
            let textX = back.depthOnLeft ? (lineX - 6) : (lineX + 16);
            let wAnchorY = back.widthOnTop ? minY : maxY;
            let dAnchorX = back.depthOnLeft ? minX : maxX;

            let extStyle = "stroke='#aaa' stroke-width='0.5' stroke-dasharray='4,4' fill='none'";
            svgContent += `<line x1="${minX}" y1="${wAnchorY}" x2="${minX}" y2="${lineY}" ${extStyle} />`;
            svgContent += `<line x1="${maxX}" y1="${wAnchorY}" x2="${maxX}" y2="${lineY}" ${extStyle} />`;
            svgContent += `<line x1="${dAnchorX}" y1="${minY}" x2="${lineX}" y2="${minY}" ${extStyle} />`;
            svgContent += `<line x1="${dAnchorX}" y1="${maxY}" x2="${lineX}" y2="${maxY}" ${extStyle} />`;
...
async function executeExportBlueprint() { 
    ...
    if (dimState > 0) {
        // Brėžinys skirtas užsienio gamintojams – matmenys tik angliškai.
        // „Dimensions:" antraštė jau yra šablone, todėl lietuvišką „Išmatavimai:" pašaliname,
        // o „Baldo N išmatavimai:" / „Lova:" išverčiame.
        let bpDimHtml = document.getElementById('dimension-display').innerHTML
            .replace(/Baldo\s*(\d+)\s*išmatavimai:\s*/g, 'Item $1: ')
            .replace(/Išmatavimai:\s*/g, '')
            .replace(/Lova:/g, 'Bed:');
        document.getElementById('bp-dims').innerHTML = bpDimHtml; 
